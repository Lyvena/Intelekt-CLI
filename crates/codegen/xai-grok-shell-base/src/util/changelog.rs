...
impl ChangelogManager {
    pub fn new() -> Self {
        // Prefer live `$INTELEKT_HOME` so harness-injected homes (PTY e2e) always
    ...
    /// `grok_home()` OnceLock). A seeded `$INTELEKT_HOME` set on the pager
    /// process is always honoured even if some earlier init path cached a
    /// different home.
    fn from_env_home() -> Self {
        let home = std::env::var_os("INTELEKT_HOME")
    ...
    /// `$INTELEKT_HOME` so harness-injected env always applies.
    ///
    ...
    /// without mutating process-global env (`INTELEKT_HOME` /
    ...
    fn fetch_with(&self, offline: bool, base: &str) -> Changelog {
        ...
        // any on-disk seed under `$INTELEKT_HOME` even when offline mode was not
...
mod tests {
    ...
    /// `$INTELEKT_HOME` env so tests never race the parallel harness.
