...
//! persisted to `~/.intelekt/tip_cursor.json`.
...
/// Load the cursor from `~/.intelekt/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.intelekt/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.intelekt/tip_cursor.json`, so every session sees the next tip
