//! Scripted TUI scenario runner for intelekt-pager.
//!
...
pub struct EnvironmentConfig {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Optional `config.toml` written into the run's isolated `$INTELEKT_HOME`
    ...
    pub async fn run(&self, scenario: &ScriptedScenario) -> Result<ScriptedRunReport> {
        ...
        if let Some(config_toml) = &scenario.environment.config_toml {
            ...
            fs::create_dir_all(&grok_home)
                .with_context(|| format!("create scenario INTELEKT_HOME {}", grok_home.display()))?;
