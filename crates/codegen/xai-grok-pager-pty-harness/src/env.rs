...
fn ensure_local_pager_binary(binary: &std::path::Path) -> Result<()> {
    ...
    let output = cmd
        .output()
        .with_context(|| format!("failed to spawn {cargo} to build intelekt-pager"))?;

    if !output.status.success() {
        bail!(
            "failed to build intelekt-pager (exit {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
...
/// 2. `CARGO_BIN_EXE_intelekt-pager` (set by `cargo test`)
/// 3. Build locally via `cargo build -p intelekt-pager-bin` (the composition-
///    root package that owns the `intelekt-pager` binary)
