...
> **Note:** Intelekt CLI applies personas through subagent resolution and roles, not through a `spawn_subagent` parameter. The main agent does not pass a persona name when it spawns a child.
...
When a persona applies, Intelekt CLI resolves the effective model and reasoning effort in this order, highest priority first:
...
Intelekt CLI manages worktrees through the `x.ai/git/worktree/*` extension methods, including an apply operation that merges changes back into the main working directory.
...
[subagents.models]
explore = "intelekt-cli"               # route explore to a specific model
```
...
Intelekt CLI shows running and finished work in side panes on the agent screen:
