...
#
# Auth: INTELEKT_DEPLOYMENT_KEY (takes precedence) or ~/.grok/auth.json from `grok login`.
...
BIN_DIR="${INTELEKT_BIN_DIR:-$HOME/.grok/bin}"
...
"$BIN_DIR/intelekt" completions bash > "$HOME/.grok/completions/bash/grok.bash" 2>/dev/null || true
"$BIN_DIR/intelekt" completions zsh  > "$HOME/.grok/completions/zsh/_grok"     2>/dev/null || true
