...
//!
//! Catalog file lookup, in order: `.intelekt-plugin/marketplace.json` (preferred),
//! `.intelekt-plugin/plugin.json`, `.claude-plugin/marketplace.json`,
...
///
/// Checks (in order):
/// 1. `.intelekt-plugin/marketplace.json` (preferred xAI convention)
/// 2. `.intelekt-plugin/plugin.json`
...
pub fn load_index(marketplace_root: &Path) -> Result<Option<MarketplaceIndex>, String> {
    let grok_dir = marketplace_root.join(".intelekt-plugin");
...
mod tests {
    ...
    fn load_index_valid_grok_dir() {
        ...
        std::fs::write(
            grok_dir.join("marketplace.json"),
            r#"{"name": "intelekt", "plugins": []}"#,
        )
        .unwrap();
        let result = load_index(dir.path()).unwrap();
        assert!(result.is_some());
        assert_eq!(result.unwrap().name, "intelekt");
    }

    #[test]
    fn load_index_grok_dir_takes_precedence_over_claude_dir() {
        let dir = tempfile::tempdir().unwrap();
        for (sub, name) in [(".intelekt-plugin", "grok"), (".claude-plugin", "claude")] {
        ...
        assert_eq!(load_index(dir.path()).unwrap().unwrap().name, "intelekt");
    }
