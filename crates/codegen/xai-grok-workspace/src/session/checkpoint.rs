...
pub(crate) fn rewind_hunks_enabled() -> bool {
    intelekt_config::env_bool("GROK_WORKSPACE_REWIND_HUNKS").unwrap_or(false)
}
...
pub(crate) fn rewind_durable_enabled() -> bool {
    intelekt_config::env_bool("GROK_WORKSPACE_REWIND_DURABLE").unwrap_or(false)
}
...
mod tests {
    ...
    async fn turn_end_with_durable_flag_off_writes_nothing_to_disk() {
        ...
        handle
            .on_turn_boundary("main", TurnBoundary::rewind_finalize(0))
            .await;
        let store_root = cwd.join(".intelekt").join("rewind-checkpoints");
        assert!(
            !store_root.exists(),
