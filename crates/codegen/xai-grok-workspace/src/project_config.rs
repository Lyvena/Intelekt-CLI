...
//! `.intelekt/config.toml` files by walking from `cwd` up to the git root.
//!
...
use intelekt_agent::repo::RepoDirChain;
...
/// True when `config_path` is `$INTELEKT_HOME/config.toml` (user tier, not project).
fn is_user_grok_config_file(config_path: &Path) -> bool {
    let Some(user_home) = intelekt_config::user_grok_home() else {
        return false;
    };
...
/// Find all `.intelekt/config.toml` files from `cwd` upward to the git repo root.
...
///
/// If no git repo is found, only checks `cwd/.intelekt/config.toml`. Excludes the
/// user-global config so `cwd == $HOME` does not treat `~/.intelekt/config.toml` as
/// a project overlay.
...
/// `cwd == $HOME` does not treat `~/.intelekt/config.toml` as a project overlay.
/// `pub(crate)` — the gate (`repo_configs_present`) reaches it within this crate.
pub(crate) fn find_project_configs_in(chain_dirs: &[PathBuf]) -> Vec<PathBuf> {
    ...
    chain_dirs
        .iter()
        .rev()
        .map(|dir| dir.join(".intelekt").join("config.toml"))
...
mod tests {
    ...
    fn find_project_configs_excludes_user_grok_config_file() {
        ...
        std::fs::create_dir_all(project.join(".intelekt")).unwrap();
        std::fs::write(project.join(".intelekt/config.toml"), "# project\n").unwrap();
