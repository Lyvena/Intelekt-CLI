//! Hub [`AuthProvider`] from `~/.intelekt/auth.json` for the standalone
...
fn default_auth_path() -> anyhow::Result<PathBuf> {
    let grok = intelekt_config::user_grok_home()
        .ok_or_else(|| anyhow::anyhow!("no user grok home (set $INTELEKT_HOME or $HOME)"))?;
...
/// the default credential path (`~/.intelekt/auth.json`).
pub fn provider(
    hub_url: &Url,
