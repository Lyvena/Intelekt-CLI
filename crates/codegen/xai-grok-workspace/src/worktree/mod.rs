...
//!
//! Moved from `intelekt-shell/src/session/worktree.rs` into the workspace
...
/// canonicalizes its `$HOME` fallback to match `intelekt_config::grok_home()`,
...
/// Returns `~/.intelekt/worktrees/<repo_slug>` for the given git root.
///
...
/// Resolves the worktree base directory (`~/.intelekt/worktrees/<repo_name>`)
/// for a given source path, correctly handling grok-managed worktrees.
///
/// When `source_path` is already under `~/.intelekt/worktrees/<repo>/...`, the
...
/// Walk up from `cwd` (staying within `~/.intelekt/worktrees/`) to its registered
/// worktree record.
///
...
/// returns `None` (without DB I/O) for paths outside `~/.intelekt/worktrees/`.
...
///
/// When the source path is already inside `~/.intelekt/worktrees/<repo>/`, the
...
/// Scan `~/.intelekt/worktrees/<repo_name>/` for subdirectories not tracked
...
mod tests {
    ...
    /// Point `INTELEKT_HOME` at an isolated tempdir (`resolve_grok_home` re-reads
    ...
    /// and restores `INTELEKT_HOME` on drop (before releasing the lock), so the
