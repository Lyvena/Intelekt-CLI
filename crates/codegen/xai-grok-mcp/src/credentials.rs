...
//!
//! Credentials are stored in `$INTELEKT_HOME/mcp_credentials.json`, keyed by a
...
/// File name for the credential store inside `$INTELEKT_HOME`.
const CREDENTIALS_FILENAME: &str = "mcp_credentials.json";

/// On-disk credential store: `$INTELEKT_HOME/mcp_credentials.json`.
///
...
impl McpCredentialStore {
    ...
    /// Load the credential store from the default path (`$INTELEKT_HOME/mcp_credentials.json`).
    ///
    ...
    pub fn save_default(&self) -> Result<()> {
        let path = Self::default_path().ok_or_else(|| {
            McpCredentialError::Other("no user grok home (set $INTELEKT_HOME or $HOME)".into())
        })?;
        self.save_to(&path)
    }
    ...
    /// Default path: `$INTELEKT_HOME/mcp_credentials.json`.
    fn default_path() -> Option<PathBuf> {
        Some(intelekt_config::user_grok_home()?.join(CREDENTIALS_FILENAME))
    }
}
...
mod tests {
    ...
    /// `$INTELEKT_HOME/mcp_credentials.json`. Existing credential files must keep
