...
//! Run locally:
//! ```bash
//! cargo test -p intelekt-shell --test test_summary_reasoning_effort -- --ignored
//! ```
...
/// Find `summary.json` for `session_id` under `<home>/.intelekt/sessions/` and
...
async fn test_fresh_session_persists_reasoning_effort() {
    with_local_set(|| async {
        ...
        let grok_dir = home.path().join(".intelekt");
