...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.intelekt/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.intelekt` (the writer may also be this process;
...
/// Watches `~/.intelekt/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.intelekt/config.toml`, `.mcp.json`,
...
/// `<cwd>/.intelekt/` (catches `<cwd>/.intelekt/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.intelekt/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.intelekt/config.toml` so the loop above
        // watches `<cwd>/.intelekt/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.intelekt/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.intelekt/config.toml` trigger a
    ...
    /// `<cwd>/.intelekt/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.intelekt/` does not yet
/// exist at session-open time, the `.intelekt/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.intelekt/` followed by a
/// write to `<cwd>/.intelekt/config.toml` will NOT be observed — the
...
fn watch_cwd_dirs(debouncer: &mut Debouncer<AccessFilteredWatcher>, cwd: &Path) {
    ...
    let grok_dir = cwd.join(".intelekt");
...
/// expected for a freshly-opened session whose `<cwd>/.intelekt/` hasn't
...
/// Watches skill directories (`~/.intelekt/skills/`, `<repo>/.intelekt/skills/`, etc.)
...
/// recursively: `grok_home` (`~/.intelekt`, or `$INTELEKT_HOME`) or a known vendor dir
/// directly under `$HOME` ([`HOME_VENDOR_DIRS`]).
///
/// These hold large non-skill trees — `~/.intelekt` alone has `worktrees/`,
...
const HOME_VENDOR_DIRS: &[&str] = &[".intelekt", ".agents", ".claude", ".cursor"];
...
/// `~/.intelekt` these exist at startup; later creation is caught on restart).
...
impl SkillsFileWatcher {
    ...
    ///
    /// Uses [`collect_skill_config_dirs`](intelekt_agent::prompt::skills::collect_skill_config_dirs)
    ...
    pub fn start(
        ...
        let dirs_to_watch = intelekt_agent::prompt::skills::collect_skill_config_dirs(
            cwd,
            monorepo_user_dir,
            &grok_home,
            config_paths,
...
mod tests {
    ...
    fn is_global_config_dir_matches_only_grok_home_and_vendor_dirs() {
        ...
        assert!(!g(&home.join("repo").join(".intelekt")));
    }

    /// Regression for the ~/.intelekt inotify-exhaustion / worktree-noise bug: a
    /// `SKILL.md` under a sibling subtree (e.g. `~/.intelekt/worktrees/`) must not
    ...
    /// A write to `<cwd>/.intelekt/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".intelekt");
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.intelekt/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.intelekt/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".intelekt");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
