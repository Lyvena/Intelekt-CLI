//! Shim — see `intelekt_telemetry::instrumentation` for the implementation.
//!
...
//!   (i.e. `intelekt_shell::instrumentation_timer!`). Keeping the macro here
...
pub use intelekt_telemetry::instrumentation::{
...
///
/// Stays in shell so callers can keep calling `intelekt_shell::instrumentation::finalize_and_exit`.
...
///
/// Macro stays in shell so `$crate` continues to resolve to `intelekt_shell`
/// for the 12+ existing call sites that spell it as
/// `crate::instrumentation_timer!(...)` or `intelekt_shell::instrumentation_timer!(...)`.
/// The macro body delegates to types and functions in
/// `intelekt_telemetry::instrumentation`.
#[macro_export]
