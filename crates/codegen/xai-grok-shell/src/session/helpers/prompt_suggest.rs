...
/// prediction.
pub(crate) const DEFAULT_SUGGEST_MODEL: &str = "intelekt-cli-0.1";
...
/// (`intelekt-cli-0.1`) is API-key-only and excluded from OAuth catalogs, so
...
mod tests {
    ...
    fn effective_model_default_requires_catalog() {
        ...
        // OAuth catalogs exclude intelekt-cli-0.1 → skip the request entirely,
