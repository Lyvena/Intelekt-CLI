...
- **MCP tool output truncation** can now be set per-repo in `.intelekt/config.toml`.
...
- **File links and media** now work for worktree sessions under ~/.intelekt/worktrees/.
...
- **Plan files** now default to.intelekt/plan.md to match Grok conventions.
...
- **Fixed git clone failures** on Windows when the CLI tries to clone marketplace plugins into ~/.intelekt.
...
- **`grok --debug`** now produces per-session log files under ~/.intelekt/debug/ even with a leader process.
...
- **Background tasks** started inside subagents now continue running after the subagent session ends.
- **Image description** now reliably uses the intelekt-cli model instead of falling back to the active session model.
...
- **API-key users** no longer see unavailable models such as intelekt-cli that 404 on selection.
...
- **Model list** no longer gets stuck on only intelekt-cli after waking from sleep on macOS.
...
- **Fixed model list** getting stuck on only intelekt-cli after waking from sleep on macOS by refreshing the catalog on every auth token update.
...
- **API key users** no longer see unavailable models like intelekt-cli in the model list.
...
- **Model picker** now recovers automatically after sleep/resume or temporary network issues instead of staying stuck on Intelekt CLI.
...
- **New `intelekt-cli-v2` agent** available in config.toml for prompt/tool testing.
...
- **New `intelekt-cli-v2` agent** option in config.toml for testing prompt and tool changes.
...
- **Best-of-N mode** for intelekt-cli
...
- **Reasoning effort settings** no longer incorrectly apply to unsupported models like intelekt-cli.
...
- **Reasoning effort settings** no longer incorrectly apply to models without support, like intelekt-cli.
...
- **intelekt-cli model** now defaults to Chat Completions API.
...
- **New `intelekt-cli-orchestrator` agent** uses a lead model to coordinate subagents for builds.
...
- **New `intelekt-cli-orchestrator`** coordinates subagents for complex builds.
...
- **Import Claude settings** from `.claude/` into `.intelekt/config.toml` via welcome screen ('i') or `/import-claude`.
...
- **X Premium and Premium+** tiers now grant access to Intelekt CLI.
...
- **Model picker** now displays config key names like `intelekt-cli` for aliased models.
...
- **Unified logs** now saved to `~/.intelekt/logs/unified.jsonl` with **Download Logs** in desktop.
...
- **Bundled agents** discovered from ~/.intelekt/bundled/agents/ with lowest precedence after project/user/built-in.
...
- **Enterprise managed configs** via ~/.intelekt/managed_config.toml/requirements.toml with `grok inspect` and `grok setup`.
...
- **Per-hook enable/disable toggles** via ~/.intelekt/disabled-hooks file and 'e' key with **multi-line j/k navigation fix**.
...
- **LSP integration** injects diagnostics as reminders and exposes opt-in code intelligence tools via `.intelekt/lsp.json`.
...
- **Dynamic nested skill discovery** in `.intelekt/skills/` subdirs with runtime system reminder announcements.
...
- **Configurable permission policies** enable automatic allow/deny rules via ~/.intelekt/config.toml before existing checks.
...
- **Dynamic discovery of nested skills** from.intelekt/skills/ subdirs at runtime via reminders.
...
- **Discover skills from**.claude/skills directories alongside.intelekt/skills.
...
- **ACP x.ai/auth/logout** removes scopes from ~/.intelekt/auth.json.
...
- **Help skill reads ~/.intelekt/config.toml** to answer MCP server and model configuration queries.
...
- **Bundled /help skill** extracts README.md to ~/.intelekt/ for slash command and model self-help.
...
- **Subagent persona support** enables layered instructions from config.toml and.intelekt/personas/*.toml with fail-closed resolution.
...
- **ACP worktree management** adds list/show/gc/db methods with filters and dry-run.
- **ACP logout method** removes scopes from ~/.intelekt/auth.json.
- **Discovers skills from.claude/skills** alongside.intelekt/skills across local, repo, and user directories.
...
- **User-defined subagents** via .intelekt/agents/*.md files with per-subagent config toggles and dynamic Task tool descriptions.
...
- **User-defined subagents** via.intelekt/agents/*.md with config toggles.
...
- **Hooks system** executes custom scripts for pre/post-tool and session events from ~/.intelekt/hooks/ with deny-wins trust controls.
...
- **Leader auto-update pre-downloads the binary** before shutdown and resolves the ~/.intelekt/bin symlink at spawn time.
...
- **npm installs now use a canonical Grok binary path** via postinstall copy to `~/.intelekt/bin/grok`, preventing installer conflicts and version confusion.
