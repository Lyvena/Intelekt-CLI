...
2. **Scope** - present the user with two options:
   - **Project** (Recommended): `<repo-root>/.intelekt/skills/<name>/SKILL.md` - available only in this repo, shareable with teammates
   - **User**: `~/.intelekt/skills/<name>/SKILL.md` - available in all projects
...
Where `<SKILL_DIR>` is:
- User scope: `~/.intelekt/skills/<name>`
- Project scope: `<repo-root>/.intelekt/skills/<name>`
