...
//!
//! Plugins from project directories (`.intelekt/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.intelekt/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.intelekt/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.intelekt/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_grok_home() so a project's `.intelekt/trusted-plugins` is never
        // read as the user trust store when neither INTELEKT_HOME nor a home dir resolves.
        let Some(grok) = intelekt_config::user_grok_home() else {
            return Self {
                trusted: HashSet::new(),
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.intelekt/trusted-plugins`.
    ...
    /// rewrites `~/.intelekt/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.intelekt/trusted-plugins`.
