...
in `.intelekt/agents/` (project-level) or `~/.intelekt/agents/` (user-level).
...
let def = AgentDefinition::from_file(".intelekt/agents/code-reviewer.md")?;
...
// Find all .md files in .intelekt/agents/ directories
...
1. **Project-level** (highest priority): `.intelekt/agents/*.md` — walk
   from `cwd` up to the git repository root. Files found closer to
   `cwd` take priority.
2. **User-level**: `~/.intelekt/agents/*.md`
...
example, a project `.intelekt/agents/code-reviewer.md` shadows a
...
|---|---|---|
| `intelekt-cli` | extend | Default agent for software engineering tasks |
