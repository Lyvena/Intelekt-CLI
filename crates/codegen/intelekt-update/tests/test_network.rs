...
use intelekt_update::auto_update::{download_silent, download_with_progress};
use intelekt_update::version::fetch_gcs_version_from_base;
...
async fn download_silent_writes_body_to_dest() {
    ...
    let dest = tmp.path().join("intelekt");
...
async fn download_silent_publishes_executable() {
    ...
    let tmp = tempfile::tempdir().unwrap();
    let dest = tmp.path().join("intelekt-0.1.181-linux-x86_64");
    download_silent(&format!("{}/bin", server.uri()), &dest)
        .await
        .unwrap();
...
async fn download_silent_to_nonexistent_parent_dir_fails() {
    ...
    let dest = tmp.path().join("missing-subdir").join("intelekt");
    let err = download_silent(&format!("{}/x", server.uri()), &dest)
        .await
        .unwrap_err();
    let msg = format!("{err:#}").to_lowercase();
