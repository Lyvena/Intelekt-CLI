...
//! `intelekt-shell` and `intelekt-pager` can depend on it without pulling
//! in domain logic.
//!
...
/// Plugin scope.
///
/// Maps from `PluginScope` in `intelekt-agent`. Variant renames:
...
///
/// Maps from `PluginOrigin` in `intelekt-agent`. Optional on [`PluginInfo`]
...
/// Hook event type.
///
/// Maps from `HookEventName` in `intelekt-hooks`. The source type's
...
pub struct HookInfo {
    ...
    /// Whether this hook is disabled via ~/.intelekt/disabled-hooks.
    #[serde(default)]
    pub disabled: bool,
}
...
mod tests {
    ...
    fn hooks_action_serde_roundtrip() {
        let action = HooksAction::Add {
            path: "/home/user/.intelekt/hooks".into(),
        };
    ...
    fn hook_info_camel_case_fields() {
        let hook = HookInfo {
            ...
            url: None,
            timeout_ms: 5000,
            source_dir: "/home/user/.intelekt/hooks".into(),
            disabled: false,
        };
    ...
    fn plugin_info_camel_case_fields() {
        let plugin = PluginInfo {
            ...
            root: "/home/user/.intelekt/plugins/test-plugin".into(),
