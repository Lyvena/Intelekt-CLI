...
//! its own `OnceLock<INTELEKT_HOME>`. The helpers below ensure the per-binary
//! initialization is identical: same env-var set, same isolation guarantees,
//! same reset between tests.
//!
//! Mirrors the INTELEKT_HOME isolation pattern used in other integration tests.
//!
//! ## Usage
//!
//! ```ignore
...
//! #[serial_test::serial]
//! async fn my_test() {
//!     let _ = test_home();   // initializes INTELEKT_HOME once per binary
...
// ─────────────────────────────────────────────────────────────────────────────
// INTELEKT_HOME isolation
// ─────────────────────────────────────────────────────────────────────────────

/// Returns a process-wide test `INTELEKT_HOME`, initialized exactly once per test
/// binary. Once initialized, `intelekt_config::grok_home()` will resolve to
...
/// Wipe state in `INTELEKT_HOME` between tests so each test sees a clean home.
...
/// Minimal [`intelekt_update::UpdateConfig`] for install tests.
pub fn make_update_config(channel: &str) -> intelekt_update::UpdateConfig {
    intelekt_update::UpdateConfig {
...
/// Backdate every file in `INTELEKT_HOME/downloads` by ~2 hours.
///
