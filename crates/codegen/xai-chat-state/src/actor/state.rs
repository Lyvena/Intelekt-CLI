...
pub fn estimate_tool_definition_tokens(td: &intelekt_sampling_types::ToolDefinition) -> u64 {
...
pub fn estimate_tool_definitions_tokens(tds: &[intelekt_sampling_types::ToolDefinition]) -> u64 {
...
pub fn estimate_item_tokens(item: &ConversationItem) -> u64 {
    ...
    match item {
        ...
        ConversationItem::Reasoning(r) => {
            ...
            let text_bytes = intelekt_sampling_types::reasoning_item_text(r).len();
...
/// intelekt-cli's [`ItemTokenCounter`](intelekt_compaction::ItemTokenCounter)
/// for the shared compaction engine: the bytes/4 estimate intelekt-cli already
...
/// intelekt-cli estimates instead, reusing [`estimate_item_tokens`] so the
/// per-variant arithmetic (images, reasoning blobs, tool-call args) stays in
/// one place.
pub struct EstimatedItemTokenCounter;

impl intelekt_compaction::ItemTokenCounter<ConversationItem> for EstimatedItemTokenCounter {
...
mod tests {
    ...
    fn estimated_item_token_counter_matches_estimate_item_tokens() {
        use intelekt_compaction::ItemTokenCounter;
    ...
    fn estimate_tool_definition_tokens_counts_name_desc_params() {
        ...
        let td = intelekt_sampling_types::ToolDefinition::function(
            "search",
            Some("find a file"),
            serde_json::json!({}),
        );
    ...
    fn estimate_tool_definitions_tokens_sums_across_slice() {
        let a = intelekt_sampling_types::ToolDefinition::function(
            "a",
            None::<&str>,
            serde_json::json!({}),
        );
        let b = intelekt_sampling_types::ToolDefinition::function(
            "b",
            None::<&str>,
            serde_json::json!({}),
        );
