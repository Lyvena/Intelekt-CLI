...
use tokio::sync::mpsc;
use intelekt_sampling_types::{ConversationItem, SamplingConfig};
...
async fn update_sampling_config_is_queryable() {
    ...
    assert_eq!(config.model, "intelekt-3");
...
async fn build_request_with_tool_definitions() {
    use intelekt_sampling_types::ToolSpec;
...
async fn build_request_uses_sampling_config() {
    ...
    assert_eq!(request.model, Some("intelekt-3".to_string()));
...
/// Simulates the exact sequence that `intelekt-shell`'s `execute_tool_calls`
...
async fn parallel_tool_calls_accept_first_reject_second_skip_third() {
    ...
    let assistant_with_tools =
        ConversationItem::Assistant(intelekt_sampling_types::AssistantItem {
...
async fn get_first_user_text_image_first_returns_none() {
    use intelekt_sampling_types::{ContentPart, UserItem};
...
async fn get_last_model_metadata_returns_both_fields() {
    let h = TestHarness::with_conversation(vec![
        ...
        ConversationItem::Assistant(intelekt_sampling_types::AssistantItem {
            content: "hello".into(),
            tool_calls: vec![],
            model_id: Some("intelekt-4.5".into()),
            model_fingerprint: Some("fp_abc123".into()),
            reasoning_effort: None,
        }),
    ]);
    let meta = h.handle.get_last_model_metadata().await;
    assert_eq!(meta.resolved_model_id.as_deref(), Some("intelekt-4.5"));
...
async fn sampling_config_survives_compaction_replacement() {
    use intelekt_sampling_types::ApiBackend;
    ...
    assert_eq!(pre.model, "intelekt-cli");
    ...
    assert_eq!(pre_meta.resolved_model_id.as_deref(), Some("intelekt-4.5"));
...
/// If that model slug is e.g. "intelekt-cli" and not in the ModelState
...
async fn model_metadata_lost_after_compaction_then_recovered_on_next_turn() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::Assistant(
            intelekt_sampling_types::AssistantItem {
...
async fn context_window_downgrade_triggers_auto_compact() {
    ...
    assert_eq!(post.model, "intelekt-4.5", "model slug must not change");
    assert_eq!(
        post.api_backend,
        ApiBackend::Responses,
...
fn serialize_via_public_api(
    req: &intelekt_sampling_types::ConversationRequest,
) -> serde_json::Value {
    ...
    intelekt_sampling_types::patch_reasoning_text_types(&mut body);
...
fn assert_prefix_stable_pair(
    base: &intelekt_sampling_types::ConversationRequest,
    extended: &intelekt_sampling_types::ConversationRequest,
    label: &str,
) {
...
async fn prefix_stable_after_model_switch() {
    ...
    let new_config = SamplingConfig {
        model: "intelekt-3-mini".to_string(),
        ..test_config()
    };
...
async fn prefix_stable_after_image_pruning() {
    ...
    h.handle
        .push_assistant_response(ConversationItem::assistant("a2"));
    h.handle
        .push_user_message(ConversationItem::User(intelekt_sampling_types::UserItem {
            content: vec![
                ContentPart::Text {
                    text: "new image".into(),
                },
...
async fn prefix_stable_with_backend_tool_calls() {
    use intelekt_sampling_types::{BackendToolCallItem, BackendToolKind, rs};
