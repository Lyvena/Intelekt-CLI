...
Sample hooks for Grok. Copy to `~/.intelekt/hooks/` to enable globally, or to `<project>/.intelekt/hooks/` for project-scoped hooks (requires `/hooks-trust`).
...
**Install:**
```sh
mkdir -p ~/.intelekt/hooks/bin
cp examples/hooks/safe-shell.json ~/.intelekt/hooks/
cp examples/hooks/bin/safe-shell-guard.sh ~/.intelekt/hooks/bin/
chmod +x ~/.intelekt/hooks/bin/safe-shell-guard.sh
```
...
```sh
mkdir -p ~/.intelekt/hooks/bin
cp examples/hooks/no-recursive-grep.json ~/.intelekt/hooks/
cp examples/hooks/bin/no-recursive-grep-guard.py ~/.intelekt/hooks/bin/
chmod +x ~/.intelekt/hooks/bin/no-recursive-grep-guard.py
```
...
Appends session metadata to `~/.intelekt/session-audit.log` — event, session ID, cwd, timestamp.

**Install:**
```sh
mkdir -p ~/.intelekt/hooks/bin
cp examples/hooks/session-log.json ~/.intelekt/hooks/
cp examples/hooks/bin/session-log.sh ~/.intelekt/hooks/bin/
chmod +x ~/.intelekt/hooks/bin/session-log.sh
```
...
Logs all tool calls to `~/.intelekt/tool-activity.log` — tool name, event type, effective tool name, backgrounded status.

**Install:**
```sh
mkdir -p ~/.intelekt/hooks/bin
cp examples/hooks/tool-logger.json ~/.intelekt/hooks/
cp examples/hooks/bin/tool-logger.sh ~/.intelekt/hooks/bin/
chmod +x ~/.intelekt/hooks/bin/tool-logger.sh
```
...
Remove the JSON file from `~/.intelekt/hooks/`. The hook stops running on the next session.