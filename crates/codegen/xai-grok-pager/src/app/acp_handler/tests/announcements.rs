#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.intelekt` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.intelekt` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.intelekt` reads).
    #[test]
