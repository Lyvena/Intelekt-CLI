#!/bin/bash
#
# Intelekt CLI installer — https://x.ai/cli/install.sh
#
# Auth: INTELEKT_DEPLOYMENT_KEY (takes precedence) or ~/.intelekt/auth.json from `grok login`.
...
#   INTELEKT_DEPLOYMENT_KEY=<key> bash <(curl -fsSL https://x.ai/cli/install.sh)
#
...
# Read a token from ~/.intelekt/auth.json for the given scope key.
# Format: {"scope_url": {"key": "token"}, ...}
read_grok_token() {
    local auth_file="$HOME/.intelekt/auth.json"
    local scope="$1"
...
if [ -n "$INTELEKT_DEPLOYMENT_KEY" ]; then
    ...
    if [ -n "$OIDC_TOKEN" ]; then
        ...
        echo "Auth: using OIDC token from ~/.intelekt/auth.json." >&2
    elif [ -n "$LEGACY_TOKEN" ]; then
        AUTH_SOURCE="auth.json (legacy)"
        echo "Auth: using legacy token from ~/.intelekt/auth.json." >&2
    fi
fi
...
BASE_URL_FALLBACK="https://storage.googleapis.com/intelekt-cli-public-artifacts/cli"
DOWNLOAD_DIR="$HOME/.intelekt/downloads"
BIN_DIR="${INTELEKT_BIN_DIR:-$HOME/.intelekt/bin}"
mkdir -p "$DOWNLOAD_DIR" "$BIN_DIR"

platform="${os}-${arch}"
CHANNEL="${INTELEKT_CHANNEL:-stable}"
...
if [ -n "$AUTH_SOURCE" ]; then
    echo "Installing Intelekt $version ($platform, $AUTH_SOURCE)..." >&2
else
    echo "Installing Intelekt $version ($platform)..." >&2
fi

binary_path="$DOWNLOAD_DIR/intelekt-$platform"
artifact_base="${BASE_URL}/intelekt-${version}-${platform}"
...
echo "  Downloading intelekt ${version}..." >&2
if [ "$os" = "windows" ]; then
    if ! download_file_parallel "${artifact_base}.exe" "$binary_tmp"; then
        if ! download_file_parallel "$artifact_base" "$binary_tmp"; then
            ...
            if is_not_found "${artifact_base}.exe"; then
                echo "Error: Intelekt is not yet available for your system ($platform)." >&2
            else
...
if [ "$os" = "windows" ]; then
    ...
    for bin_name in intelekt.exe agent.exe; do
    ...
    echo "  Binary installed to $BIN_DIR/intelekt.exe and $BIN_DIR/agent.exe." >&2
else
    chmod +x "$binary_tmp"
    if ! "$binary_tmp" --version </dev/null >/dev/null 2>&1; then
        echo "Error: downloaded intelekt failed to run; keeping the existing install." >&2
        rm -f "$binary_tmp"
        exit 1
    fi
    mv -f "$binary_tmp" "$binary_path"
    # Use relative symlinks when BIN_DIR and DOWNLOAD_DIR share a parent
    # (default layout: ~/.intelekt/bin and ~/.intelekt/downloads are siblings).
    ...
    ln -sf "$link_target" "$BIN_DIR/intelekt"
    ln -sf "$link_target" "$BIN_DIR/agent"
    echo "  Binary linked to $BIN_DIR/intelekt and $BIN_DIR/agent." >&2
fi

# Generate shell completions (best-effort)
mkdir -p "$HOME/.intelekt/completions/bash" "$HOME/.intelekt/completions/zsh"
"$BIN_DIR/intelekt" completions bash > "$HOME/.intelekt/completions/bash/grok.bash" 2>/dev/null || true
"$BIN_DIR/intelekt" completions zsh  > "$HOME/.intelekt/completions/zsh/_grok"     2>/dev/null || true
...
if mkdir -p "$HOME/.config/fish/completions" 2>/dev/null; then
    "$BIN_DIR/intelekt" completions fish > "$HOME/.config/fish/completions/intelekt.fish" 2>/dev/null || true
fi

# Persist installer source and channel to config
CONFIG_FILE="$HOME/.intelekt/config.toml"
...
if [ -n "$INTELEKT_DEPLOYMENT_KEY" ]; then
    PROXY_URL="${INTELEKT_PROXY_URL:-https://cli-chat-proxy.grok.com/v1}"
    ...
    if [ -n "$AUTH_HEADER_FILE" ]; then
        ...
        printf 'Authorization: Bearer %s\n' "$INTELEKT_DEPLOYMENT_KEY" > "$AUTH_HEADER_FILE"
    ...
    if [ -n "$DEPLOY_RESPONSE" ]; then
        ...
        if [ -n "$MANAGED_CONFIG" ] && [ "$MANAGED_CONFIG" != "null" ]; then
            printf '%s\n' "$MANAGED_CONFIG" > "$HOME/.intelekt/managed_config.toml"
            echo "  Managed config applied." >&2
        else
            rm -f "$HOME/.intelekt/managed_config.toml"
        fi
        if [ -n "$REQUIREMENTS" ] && [ "$REQUIREMENTS" != "null" ]; then
            printf '%s\n' "$REQUIREMENTS" > "$HOME/.intelekt/requirements.toml"
            echo "  Requirements applied." >&2
        else
            rm -f "$HOME/.intelekt/requirements.toml"
        fi
    fi
fi

if [ "$os" = "windows" ]; then
    echo "Intelekt $version installed to $BIN_DIR/intelekt.exe" >&2
else
    echo "Intelekt $version installed to $BIN_DIR/intelekt" >&2
fi
...
# Try to symlink into a directory already on PATH so intelekt works immediately
...
if [ "$os" != "windows" ] && ! path_has_dir "$BIN_DIR"; then
    for candidate in "$HOME/.local/bin" "/usr/local/bin"; do
        if path_has_dir "$candidate" && [ -d "$candidate" ] && [ -w "$candidate" ]; then
            ln -sf "$BIN_DIR/intelekt" "$candidate/intelekt"
            ln -sf "$BIN_DIR/agent" "$candidate/agent"
            SYMLINK_CREATED="$candidate"
            echo "  Symlinked $candidate/grok -> $BIN_DIR/intelekt" >&2
...
# Also update shell config so ~/.intelekt/bin is on PATH for future sessions
...
if [ -n "$config_file" ]; then
    ...
    if [ "$user_shell" = "fish" ]; then
        new_block='# >>> intelekt installer >>>
fish_add_path $HOME/.intelekt/bin
# <<< intelekt installer <<<'
    elif [ "$user_shell" = "zsh" ]; then
        new_block='# >>> intelekt installer >>>
export PATH="$HOME/.intelekt/bin:$PATH"
fpath=(~/.intelekt/completions/zsh $fpath)
...
# <<< intelekt installer <<<'
    ...
    if grep -qs "intelekt installer" "$config_file" 2>/dev/null; then
        # Replace existing block in-place (strip old >>> to <<< lines, insert new)
        tmp="$config_file.tmp.$$"
        awk '
            /# >>> intelekt installer >>>/ { skip=1; next }
            /# <<< intelekt installer <<</ { skip=0; next }
            !skip { print }
        ' "$config_file" > "$tmp" && mv "$tmp" "$config_file"
    else
...
if path_has_dir "$BIN_DIR" || [ -n "$SYMLINK_CREATED" ]; then
    echo "Run 'intelekt' or 'agent' to get started!" >&2
elif [ -n "$config_file" ]; then
    echo "Restart your terminal, then run 'intelekt' or 'agent' to get started!" >&2
else
    echo "Add $BIN_DIR to your PATH, then run 'intelekt' or 'agent' to get started:" >&2
    echo '  export PATH="$HOME/.intelekt/bin:$PATH"' >&2
fi

if [ "$os" = "windows" ]; then
    echo "To use intelekt from cmd.exe or PowerShell, add %USERPROFILE%\\.grok\\bin to your PATH." >&2
fi
