...
|----------|-------|----------|-------|
| `./.intelekt/skills/`, `./.intelekt/commands/` | Local (CWD) | Highest | Current directory skills / legacy command markdown |
| `<repo_root>/.intelekt/skills/`, `…/commands/` | Repo | Medium | Shared across the repo |
| `~/.intelekt/skills/`, `~/.intelekt/commands/` | User | Lowest | Personal skills for all projects |
...
Grok deduplicates skills by name -- a higher-priority location overrides a lower one. Grok also scans `.agents/skills/` (and `commands/`) at each tier (alongside `.intelekt/`) and walks every directory between your working directory and the repo root.
...
Skill and command discovery does **not** use `.gitignore`. Paths under known skill roots (`.intelekt/`, `.agents/`, `.claude/`, `.cursor/`) always load when present on disk — teams often ignore `.claude/**` as local-only config while still expecting `/frontend`-style project commands to work. To hide a skill, use `[skills] ignore` in config (not repo ignore rules).

Grok scans the Claude and Cursor skill directories by default. To stop scanning a vendor, set its `skills` cell to `false` under `[compat.cursor]` or `[compat.claude]` in `~/.intelekt/config.toml`, or set the `GROK_CURSOR_SKILLS_ENABLED` or `GROK_CLAUDE_SKILLS_ENABLED` environment variable to `false`. See [Configuration](05-configuration.md#harness-compatibility) for details. Grok always filters out known vendor-shipped default skills (such as Cursor's `shell`, `canvas`, and `statusline`), regardless of these settings.

### Additional Skill Directories

Add directories, exclude paths, or disable individual skills via `[skills]` in `~/.intelekt/config.toml`:
...
3. **Creates the skill directory.** Grok creates the `<scope>/.intelekt/skills/<name>/` directory, plus `scripts/` or `references/` subdirectories when the skill needs them.
...
- **Project** (`<repo_root>/.intelekt/skills/<name>/`) -- available only in this repository and shareable with teammates through version control. Grok recommends this scope inside a git repository.
- **User** (`~/.intelekt/skills/<name>/`) -- available across all your projects.
...
```
/local:commit        # The "commit" skill from ./.intelekt/skills/
/user:commit         # The "commit" skill from ~/.intelekt/skills/
```
...
5. **Version-control project skills.** Commit `.intelekt/skills/` to your repository so the whole team benefits. User skills in `~/.intelekt/skills/` stay personal and unshared.
