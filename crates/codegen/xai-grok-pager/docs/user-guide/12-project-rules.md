...
| Location | Notes |
|----------|-------|
| `<dir>/.intelekt/rules/` | Always scanned |
...
1. **Global rules**: `~/.intelekt/` (applies to all projects)
...
As top-level instruction files, Grok discovers only the recognized filenames listed under [Supported File Names](#supported-file-names) — not custom names such as `AGENTS.local.md` or `notes.md`. (Inside a rules directory such as `.intelekt/rules/`, every `*.md` file is loaded regardless of name.)

---

## The .intelekt/ Project Directory

Beyond AGENTS.md files, the `.intelekt/` directory in your project root can contain additional project-level configuration:

| Path | Purpose |
|------|---------|
| `.intelekt/config.toml` | Project-scoped MCP servers, plugins, and permission rules (other settings load only from `~/.intelekt/config.toml`) |
...
| `.intelekt/hooks/` | Project-scoped lifecycle hooks |
...
5. **Version control your rules.** Commit AGENTS.md to the repository so the whole team benefits. User-specific overrides belong in `~/.intelekt/` (global rules).
