...
MCP servers are configured in `~/.intelekt/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.intelekt/config.toml`) **or repo-level**
>   (`.intelekt/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.intelekt/config.toml` >
...
By default `grok mcp add` writes to `~/.intelekt/config.toml` (`--scope user`). Use `--scope project` to write to `.intelekt/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `grok mcp list` shows servers from both scopes, marking project-scoped ones with `(project)`.
...
MCP servers can be configured per-project by placing a `.intelekt/config.toml` in your repository:
...
Grok walks from the current directory up to the git repo root, loading `.intelekt/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.intelekt/config.toml` | All projects | Lowest |
| `<repo-root>/.intelekt/config.toml` | This repository | Medium |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.intelekt/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.intelekt/config.toml`, `.intelekt/config.toml` | Always on |
...
You must authenticate OAuth-based MCP servers before you can use them. Grok stores the resulting tokens under `~/.intelekt/mcp_credentials.json`. After you edit `config.toml`, press `r` in the `/mcps` modal to refresh the server list.
...
For stdio servers, Grok captures the process's standard error to `~/.intelekt/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.intelekt/logs/mcp/filesystem.stderr.log
```
