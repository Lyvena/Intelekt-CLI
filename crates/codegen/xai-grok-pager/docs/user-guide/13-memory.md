...
|----------|-------|-------------|
| `~/.intelekt/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.intelekt/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.intelekt/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.intelekt/memory/MEMORY.md` for cross-project preferences:
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.intelekt/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.intelekt/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
```
Memory saved to ~/.intelekt/memory/MEMORY.md
```
...
To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.intelekt/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.intelekt/memory/` for external edits and reindex |
...
By default, Grok watches `~/.intelekt/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
```bash
ls ~/.intelekt/memory/
cat ~/.intelekt/memory/MEMORY.md
$EDITOR ~/.intelekt/memory/MEMORY.md
```
