...
Intelekt CLI supports **hooks** (event-driven shell commands) and **plugins** (bundles of skills, agents, hooks, and MCP servers). Both are managed through a unified modal interface.
...
- **Global hooks** — from `~/.intelekt/hooks/`
- **Project hooks** — from `.intelekt/hooks/` in your repo
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.intelekt/settings.json` or `~/.claude/settings.json`
...
Sources land in `~/.intelekt/config.toml`:
...
Or in `~/.intelekt/settings.json` / `~/.claude/settings.json`:
