...
//! after every tool call to check for SKILL.md files in `.intelekt/skills/`,
...
/// Directories that contain skill definitions (`.intelekt/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".intelekt", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.intelekt/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.intelekt/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.intelekt/) are outside the git root so the walk breaks early.
