//! LSP server configuration from `.intelekt/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".intelekt").join("lsp.json");
...
/// Load LSP server configs from `~/.intelekt/lsp.json` and `<cwd>/.intelekt/lsp.json`.
