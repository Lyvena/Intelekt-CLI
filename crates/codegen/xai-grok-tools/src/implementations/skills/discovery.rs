...
///
/// `skills` is the standard layout (`.intelekt/skills/`, `.claude/skills/`,
/// `.cursor/skills/`). The product-specific `skills-cursor/` layout is no
/// longer scanned — it pulled vendor default skills into Intelekt CLI sessions.
...
/// name (e.g. `~/.intelekt/skills/shell`) is NOT dropped — only skills physically
...
pub fn parse_skill_files(skill_files: Vec<(PathBuf, SkillScope)>) -> Vec<SkillInfo> {
    ...
    // per-vendor toggle, so vendor builtins never leak into Intelekt CLI.
...
/// `cwd` (exclusive). At each directory, checks for `.intelekt/skills/`,
...
pub fn discover_skills_for_paths(
    ...
    let mut config_dir_names: Vec<&str> = vec![".intelekt", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.intelekt is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.intelekt/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.intelekt/ → kept (user content).
        let grok_shell = tmp.path().join(".intelekt").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.intelekt/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".intelekt").join("skills").join("grok-dyn");
