...
//! config helper `intelekt-shell::util::config::resolve_search_tools_enabled`
...
//! (release builds, self-extracted to `~/.intelekt/vendor/<name>-<ver>-<target>`) →
//! `~/.intelekt/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.intelekt/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.intelekt/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.intelekt/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.intelekt/vendor` and the extracted `bfs` runs.
