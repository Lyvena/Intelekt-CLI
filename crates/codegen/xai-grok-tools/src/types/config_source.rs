...
pub enum ConfigSource {
    ...
    Builtin,
    /// Bundled skill shipped with the binary (extracted to ~/.intelekt/skills/
    /// or injected via bundled skill dirs).
    Bundled { path: PathBuf },
    /// Server-synced (e.g. ~/.intelekt/server-skills from the skill store).
    Server { path: PathBuf },
    /// Project-scoped: cwd/.intelekt/ or cwd/.claude/.
    Project { path: PathBuf },
    /// User-scoped: ~/.intelekt/ or ~/.claude/.
    User { path: PathBuf },
