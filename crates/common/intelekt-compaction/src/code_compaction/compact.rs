//! intelekt-cli's full-replace compaction pass.
//!
...
//! ```
//!
//! Per-harness concerns stay in the product host (for example `intelekt-shell`): the triggers, the
...
pub struct FullReplaceContext<T> {
    ...
    /// turn). intelekt-cli keeps this; pass empty to drop it.
...
///
/// Returned by [`sample_full_replace_summary`] for harnesses (intelekt-cli's
...
/// Run intelekt-cli's full-replace compaction pass and return the rebuilt
...
///
/// This is the seam intelekt-cli's shell uses: it drives the verbatim → fitted →
    ...
    O: FullReplaceObserver + ?Sized,
{
    ...
    let prompt = CompactionPrompt {
        // intelekt-cli appends the summarization prompt as the final user
...
mod tests {
    ...
    /// returns a structured summary must produce intelekt-cli's exact compacted
