...
//!
//! The canonical `sample → classify → retry` loop, used by **both** intelekt-cli's
...
//! the winning summary (intelekt-cli cleans in its assembler; intra cleans via
...
pub enum SampleRetryError {
    ...
    Failure {
        ...
        /// signal the intelekt-cli host uses to step down its input size).
